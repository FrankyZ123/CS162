...
import itertools
import gettext
from random_generators import generators
...
class CardDeck(object):
    ...
    def pop_rand(self, rand_method):
        ...
        rand_num = rand_method.random_int()
...
def game(rand_method):
    ...
    for i in range(2):
        card = deck.pop_rand(rand_method=rand_method)
        my_hand.append(card)
        card = deck.pop_rand(rand_method=rand_method)
...
if __name__ == '__main__':
    ...
    print()
    print('BlackJack Settings') 
    print('-' * 30)
    rand_method = input("Enter your prefered random number generator (type one from this list: {}): ".format(', '.join(types.generators)))
    
    while rand_method not in types.generators:
        print("Oops, looks like you spelled something wrong; let's try that again.")
        rand_method = input("Enter your prefered random number generator (type one from this list: {}): ".format(', '.join(types.generators)))
    
    print("Cool, we'll use {} as the random number generator.".format(rand_method))
    print('-' * 30)
    print()

    game(types.generators[rand_method])
